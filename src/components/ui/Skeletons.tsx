...
/** Table skeleton — matches DataGrid layout */
export function TableSkeleton({ rows = 8, cols = 5 }: { rows?: number; cols?: number }) {
  return (
    <Paper variant="outlined" sx={{ borderRadius: 2, overflow: "hidden" }}>
      ...
      {/* Header */}
      <Box sx={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 2, px: 2, py: 1.5, bgcolor: "action.hover" }}>
      ...
      {Array.from({ length: rows }).map((_, r) => (
        <Box key={r} sx={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 2, px: 2, py: 1.5, borderTop: "1px solid", borderColor: "divider" }}>
          {Array.from({ length: cols }).map((_, c) => (
            <Skeleton key={c} variant="text" height={16} width={c === 0 ? "80%" : c === cols - 1 ? "40%" : "60%"} />
          ))}
        </Box>
      ))}
    </Paper>
