...
const FilterPanel = makeReportFilterPanel({ showClient: true, showMatter: true, showUser: true, showDateRange: true })
...
export default function MarginErosionPage() {
  ...
  async function emailExcel() {
    try {
      toast.success(await reportsApi.requestMarginErosionExcel())
    } catch {
  ...
  return (
    <PageShell
      ...
      description="Profitability and margin by matter"
      action={(
    ...
    >
          ...
          <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
            ...
            <ApexChart
              ...
              options={{
                ...
                xaxis: { categories: rows.map(r => String(r.matterTitle ?? r.matterName ?? "")), labels: { style: { fontSize: "10px" } } },
                colors: [...rows.map(r => Number(r.marginRate ?? 0) < 40 ? "#DC2626" : Number(r.marginRate ?? 0) < 60 ? "#F59E0B" : "#22C55E")],
                dataLabels: { enabled: false },
                plotOptions: { bar: { borderRadius: 4, columnWidth: "55%", distributed: true } },
                legend: { show: false },
                grid: { strokeDashArray: 4 },
                yaxis: { max: 100, labels: { formatter: (v: number) => `${v.toFixed(0)}%` } },
                annotations: { yaxis: [{ y: avgMargin, borderColor: "#0F3C6E", label: { text: `Avg ${avgMargin.toFixed(1)}%`, style: { color: "#fff", background: "#0F3C6E" } } }] },
              }}
            />
          </Paper>
      ...
      <DataGrid
        columns={[
          { field: "matterTitle", header: "Matter", renderCell: (v, row) => String(v || (row as Record<string, unknown>).matterName || "—") },
          ...
          { field: "margin", header: "Margin", align: "right", renderCell: v => formatCurrency(Number(v ?? 0)) },
          {
            ...
            align: "right",
            renderCell: v => (
              <Typography variant="body2" sx={{ color: Number(v) < 40 ? "error.main" : Number(v) < 60 ? "warning.main" : "success.main", fontWeight: 600 }}>
        ...
        queryFn={(p: GridParams) => reportsApi.getMarginErosion(p)}
        FilterPanel={FilterPanel}
        hasFilters
        syncWithUrl
        defaultSortBy="marginRate"
        defaultSortDir="asc"
        zebraStriping
      />
    </PageShell>
