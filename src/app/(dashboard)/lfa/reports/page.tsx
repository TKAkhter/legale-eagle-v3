...
async function fetchLfaBilling(params: GridParams) {
  ...
  const r = await axiosClient.get('/api/report/lfa/billing/v2', {
    params: { ...qp, clientId: f.clientId ?? '', fromDate: f.fromDate ?? '', toDate: f.toDate ?? '' },
  })
...
async function fetchLfaReferral(params: GridParams) {
  ...
  const qp = buildQueryParams(params, { paginationConvention: 'pageNumber-pageSize' })
  const f = params.filters ?? {}
  const r = await axiosClient.get('/api/report/lfa/referral', {
    params: { ...qp, fromDate: f.fromDate ?? '', toDate: f.toDate ?? '' },
  })
  return r.data?.data ?? r.data
}
      ...
      <Tabs tabs={[
        {
          ...
          content: (
            <DataGrid
              columns={[
                { field: 'lfaTitle', header: 'LFA', renderCell: (v, row) => String(v || (row as Record<string, unknown>).agreementNo || '—') },
                {
                  field: 'client',
                  header: 'Client',
                  renderCell: v => {
              ...
              queryKey={['reports', 'lfa', 'billing']}
              queryFn={fetchLfaBilling}
              FilterPanel={ReportFilter}
              hasFilters
              hasExport
              syncWithUrl
            />
        ...
        },
        {
          ...
          content: (
            <DataGrid
              ...
              queryKey={['reports', 'lfa', 'referral']}
              queryFn={fetchLfaReferral}
              FilterPanel={ReportFilter}
              hasFilters
              hasExport
              syncWithUrl
