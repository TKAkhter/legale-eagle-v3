  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
        ...
        <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
          <Typography variant="subtitle2" sx={{ mb: 1.5, fontWeight: 600 }}>Contacts</Typography>
          {contacts.length === 0 ? (
            <Typography color="text.secondary" variant="body2">No contacts.</Typography>
          ) : contacts.map((c, i) => {
            const email = c.email
            const phone = c.phone
            return (
              <Box key={i} sx={{ mb: 1.5, pb: 1.5, borderBottom: i < contacts.length - 1 ? "1px solid" : "none", borderColor: "divider" }}>
                <Typography variant="body2" sx={{ fontWeight: 600 }}>
                  {`${c.firstName ?? ""} ${c.lastName ?? ""}`.trim() || "—"}
                  {c.primary ? " (Primary)" : ""}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {typeof email === "string" ? email : String((email as { emailId?: string })?.emailId ?? "—")}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {typeof phone === "string" ? phone : String((phone as { phoneNo?: string })?.phoneNo ?? "—")}
                </Typography>
              </Box>
            )
          })}
