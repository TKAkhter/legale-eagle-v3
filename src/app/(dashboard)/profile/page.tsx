...
import { z } from "zod"
import {
  Box, Paper, Typography, Avatar, Button,
            ...
            <Box>
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                {(["light","dark"] as const).map(m => (
                  <Button key={m} size="small" variant={colorMode === m ? "contained" : "outlined"}
            ...
            <Box>
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                {([["en","English"],["ar","العربية"]] as const).map(([code,label]) => (
                  <Button key={code} size="small" variant={language === code ? "contained" : "outlined"}
