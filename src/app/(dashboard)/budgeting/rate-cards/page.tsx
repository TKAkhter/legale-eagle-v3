  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5" sx={{ fontWeight: 600 }}>Rate Cards</Typography>
        <Button variant="contained" startIcon={<AddIcon />}>Add Rate Card</Button>
      </Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Billing rate per hour per fee earner — used when billing type is Hourly.</Typography>
      <DataGrid
        columns={[
          { field: 'user', header: 'Fee Earner', renderCell: (v) => { const u = v as Record<string, string>; return u ? `${u.firstName ?? ''} ${u.lastName ?? ''}`.trim() : '—' } },
          { field: 'ratePerHour', header: 'Rate/Hour', align: 'right', renderCell: (v) => formatCurrency(Number(v ?? 0)) },
        ...
        queryKey={['budgeting', 'rateCards']} queryFn={fetchRateCards}
        FilterPanel={Filter} hasFilters isPaginated={false}
        emptyState={<Typography color="text.secondary">Select a fee earner to view their rate cards</Typography>}
      />
    </Box>
  )
}
