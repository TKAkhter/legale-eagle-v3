...
async function fetchUsers(params: GridParams) {
  ...
  const r = await axiosClient.get('/api/user/get',{
    params:{...qp, searchText:f.searchText??'', departmentId:f.departmentId??'', companyUserType:f.companyUserType??''}
  })
  ...
  return (
    <Box>
      ...
      <DataGrid
        ...
        queryKey={['users','list']} queryFn={fetchUsers}
        FilterPanel={UsersFilterPanel} hasFilters syncWithUrl
        rowMenuItems={(row)=>[
          { label: row.active ? 'Block User' : 'Unblock User', icon:<BlockIcon fontSize="small"/>, permission:PERMISSIONS.USERS_BLOCK, color:'error', onClick:()=>blockUser(row) }
        ]}
      />
