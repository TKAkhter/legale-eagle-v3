...
export default function DashboardPage() {
  ...
  const matCount = (history ?? []).map((d: Record<string,unknown>) => Number(d.count ?? 0))
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 700 }}>{greeting()}, {user?.firstName}</Typography>
        <Typography variant="body2" color="text.secondary">Here's what's happening at the firm today.</Typography>
      </Box>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 2, mb: 3 }}>
        ...
        <KpiCard title="Pending Tasks" value={counts?.pendingTasks} sub={counts?.overdueTasks ? `${counts.overdueTasks} overdue` : undefined} loading={l1} />
      </Box>

      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2 }}>
        <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1.5 }}>Matter Activity (Monthly)</Typography>
          {l2 ? <Skeleton height={200} /> : matCount.length > 0 && (
            <ApexChart type="area" height={200}
              series={[{ name: "Matters", data: matCount }]}
