/**
 * Send invoice for internal approval — LMS SendForApproval (POST /invoice/ap/send).
 */
...
export function InvoiceSendForApprovalDialog({ open, onClose, invoiceId, onSent }: Props) {
  const [approver, setApprover] = useState("")
  const [error, setError] = useState("")
  const [saving, setSaving] = useState(false)

  const usersQ = useQuery({
    queryKey: ["users", "min", "invoice-approve"],
    enabled: open,
  ...
  async function submit() {
    ...
    setError("")
    try {
      toast.success(await billingApi.sendForApproval(invoiceId, approver))
      onSent?.()
      onClose()
    } catch (e: unknown) {
      setError(
